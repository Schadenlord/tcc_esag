...
\midrule
\endfirsthead
\multicolumn{10}{l}{\footnotesize \textit{(continuação)}}\\\toprule
\textbf{DV} & \textbf{Bloc} & $N$ & $n_{e=0}$ & $n_{e=1}$ & $\bar{Y}_{e=0}$ & $\bar{Y}_{e=1}$ & $\sigma_{e=0}$ & $\sigma_{e=1}$ & $\Delta\bar{Y}$ \\
\midrule
\endhead
\midrule
...
\noindent\textit{\textbf{Notas:}}\ $e=0$ = público geral ($n\approx166$); $e=1$ = economistas ($n\approx17$); $\Delta\bar{Y} = \bar{Y}_{e=1} - \bar{Y}_{e=0}$; escala 0--2. Médias observadas (não ajustadas). 53 DVs.
\end{minipage}
\endgroup
