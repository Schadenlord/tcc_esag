...
\midrule
\endfirsthead
\multicolumn{11}{l}{\footnotesize \textit{(continuação)}}\\\toprule
\textbf{DV} & \textbf{Bloc} & $N$ & $\hat{\beta}_\text{econ}$ & $p$ (nom.) & $p$ (BH) & $p$ (Holm) & OR & $\hat{\beta}_\text{esp}$ & $p_\text{esp}$ & $R^2$ \\
\midrule
\endhead
\midrule
...
\noindent\textit{\textbf{Notas:}}\ $\dag$ = Hessiana singular (Firth-Ridge, $\lambda=0{,}1$); *\,$p<0,05$, **\,$p<0,01$, ***\,$p<0,001$ (nominal). BH = Benjamini-Hochberg; Holm = Holm-Bonferroni. OR = \textit{odds ratio} (logit ordenado padrão; para DVs singulares ver Tab.~Firth-Ridge). $R^2$ = McFadden.
\end{minipage}
\endgroup
