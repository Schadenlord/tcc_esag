...
\ifXeTeX
  % XeLaTeX: fontspec com TeX Gyre Termes via nome de arquivo (tectonic baixa do bundle)
  \usepackage{fontspec}
...
% [duplicate inputenc/fontenc já carregados acima; DeclareUnicodeCharacter incompatível com XeLaTeX]
...
\newcommand{\notafig}{\newline Fonte: elaboração do autor com base nos modelos da \autoref{sec:modelo-estatistico} (contrafactual da \autoref{sec:publico_contrafactual})}%

% Nota de figura com argumento: Nota + texto + Fonte centralizada (fonte 10, espaçamento simples)
\newcommand{\notafigv}[1]{%
  \par\begingroup
  \SingleSpacing\footnotesize\centering
  \textit{Nota:} #1\par
  Fonte: elaboração do autor (2025).\par
  \endgroup}
...
\newcommand{\me}[1]{Fonte: Elaborado pelo autor (#1).}
